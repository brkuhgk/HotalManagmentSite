...
class user
{
    ...
    {
        $this->db = new mysqli(DB_SERVER, DB_USERNAME, DB_PASSWORD, 'srinithDineshKarthikDatabase');
        ...
        $username = "srinithDineshKarthik";
        $password = "srinithDineshKarthikPass";

        try {
            $conn = new PDO("mysql:host=$servername;dbname=srinithDineshKarthikDatabase", $username, $password);
    ...
    {
        $sql = "SELECT room_cat, SUM(roomcount) as occupied_rooms FROM rooms WHERE room_id IN (SELECT DISTINCT room_id FROM rooms WHERE (checkin <= '$checkin' AND checkout >='$checkout') OR (checkin >= '$checkin' AND checkin <='$checkout') OR (checkin <= '$checkin' AND checkout >='$checkin')) GROUP BY room_cat";
       $check = mysqli_query($this->db, $sql)  or die(mysqli_connect_errno() . "Query Doesnt run");
       return $check;
    }

    public function booknow($confirmation_id, $email,$address,$city,$state,$checkin, $checkout, $name, $phone, $roomname, $price, $roomcount, $adults, $children)
    {
        ...
        if (mysqli_num_rows($check) > 0) {
            ...
            $sql2 = "UPDATE rooms  SET conf_id='$confirmation_id', email='$email', address='$address', city='$city', state='$state', checkin='$checkin', checkout='$checkout', name='$name', phone='$phone', book='true',totalprice='$price',roomcount='$roomcount',adults='$adults',children='$children' WHERE room_id=$id";
